...
class Environment:
    ...
    def add_new_vehicles_by_number(self):
        ...
        self.c = np.zeros(self.n_Veh)
        self.t = np.zeros(self.n_Veh)
        self.b = np.zeros(self.n_Veh)
        self.trans = np.zeros([self.n_Veh,self.task_num])
        self.exe_delay = np.zeros([self.n_Veh,self.task_num])
        
        for i in range(len(self.vehicles)):
            self.task = np.append(self.task, self.vehicles[i]['compute_task'])
            self.ve_v[i] = self.vehicles[i]['velocity']
            self.ve_l[i] = self.vehicles[i]['position'][0]
            self.c[i] = self.unit_cycle
            self.t[i] = self.t_limit
            self.b[i] = self.band_width        
    ...
    def reward(self, action):
        reward = np.array([])
        self.cmp_res = np.zeros([self.n_Veh,self.task_num])
        T_exe = self.delay(action)
        delay_limit = np.array([self.t_limit for x in range(self.task_num)])

        for i in range(self.n_Veh):
            for j in range(self.task_num):
                self.cmp_res[i][j] = action[i][j+self.task_num]
        
        used_fre = np.sum(self.cmp_res + self.delta_f)
        alpha = 0.5
        for i in range(self.n_Veh):
                v_reward = 10*np.mean((T_exe[i] - self.t_limit)) - alpha*(np.sum(self.cmp_res+self.delta_f)/self.n_Veh-self.bs_max_fre/self.n_Veh)            
